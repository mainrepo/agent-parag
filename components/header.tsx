...
import {AgentPic} from '../components/pics';
        ...
        <><div className="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
            <div className="relative flex items-center space-x-4">
                <div className="relative">
                    ...
                    <AgentPic alt="Agent Parag" className="w-13 h-13 sm:w-16 sm:h-16 rounded-full transform -scale-x-100" />
                </div>
                <div className="flex flex-col leading-tight">
                    <div className="text-2xl mt-1 flex items-center">
                        <span className="text-gray-700 mr-3">{props.name}</span>
                    </div>
                    <div className="text-1xl mt-1 flex items-center flex-wrap">
                        {description.map((word: string, index: number) => (
                            (<span key={index} className="capitalize text-sm text-gray-500">{word}&nbsp;</span>)
                        ))}
                    </div>
                </div>
            </div>
